  ...
  useEffect(() => {
    ...
    async function loadPriceHistory() {
      try {
        ...
        if (isMounted && Array.isArray(json) && json.length >= 2) {
          const formatted = json.map((point: PricePoint) => ({
            ...point,
            formattedDate: formatPriceDate(point.date),
          }));
          setData(formatted);

          const prices = json.map((point: PricePoint) => point.price);
