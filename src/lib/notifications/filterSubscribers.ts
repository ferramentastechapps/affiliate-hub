 ...
 * 
 * 1. CUPONS: Todos os inscritos recebem notificações de cupons (hasCoupon === true).
 * 2. OFERTAS / PROMOÇÕES:
 *    - Se o assinante cadastrou palavras-chave (customInterests) e o produto bate com alguma delas -> Recebe.
 *    - Se o assinante cadastrou categorias e a oferta bate com alguma categoria -> Recebe.
 *    - Se o assinante não cadastrou filtros específicos (ou acabou de instalar) -> Recebe ofertas gerais (fallback).
 *    - Se o assinante cadastrou filtros específicos e a oferta não bate com nenhum deles -> Não recebe.
 */
export function filterSubscribers<T extends Subscriber>(
  ...
  return subscriptions.filter((sub) => {
    // Regra 1: Notificações com CUPOM -> Todos os assinantes recebem!
    if (hasCoupon) {
      return true;
    }

    // Regra 2: Sem preferências definidas (legado ou recém-instalado) -> Recebe sempre.
    if (!sub.preferences) {
      return true;
    }
    ...
    // Regra 4: Categorias Específicas
    ...
    // Regra 5: Assinante sem filtros restritivos (recebe as ofertas gerais)
    if (!hasSpecificCats && !hasSpecificInterests) {
      return true;
    }

    // Regra 6: Fallback geral para disparos informativos/administrativos sem categoria ou produto
    if (!category && (!categories || categories.length === 0) && !productName) {
      return true;
    }

    // O assinante tem filtros específicos, mas a oferta atual não corresponde a nenhum deles
    return false;
  });
}
