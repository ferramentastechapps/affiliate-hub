...
scp -i "$env:USERPROFILE\.ssh\id_rsa" next_build.tar.gz root@212.85.10.239:~/affiliate-hub/
scp -i "$env:USERPROFILE\.ssh\id_rsa" $DeployScript root@212.85.10.239:~/deploy.sh

Write-Host "Executando script na VPS..." -ForegroundColor Cyan
ssh -i "$env:USERPROFILE\.ssh\id_rsa" root@212.85.10.239 "bash ~/deploy.sh && rm ~/deploy.sh"
